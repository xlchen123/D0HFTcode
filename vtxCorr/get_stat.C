...
void get_stat() {
    ...
    TFile* fin = new TFile("data/vtxStat_default.root");
    for(int icent=0; icent<ncentAll_vtx; icent++) {
        ...
        herr[icent] = (TH1D*)fin->Get(Form("statError_%s",nameCentAll_vtx1[icent]));
    ...
    float sys;
    for(int icent=0; icent<ncent; icent++) {
        out.open(Form("data/vtxStat_%s.txt",nameCent1[icent]));
