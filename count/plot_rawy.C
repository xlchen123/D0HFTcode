...
void plot_rawy()
{
    ...
    for(int icent=0; icent<ncent; icent++) {
        ...
        for(int ipt=0; ipt<npt; ipt++) {
            ...
            hInvMassMixUL[ipt]->SetLineWidth(4);
            hInvMassLS[ipt]->Draw("HISTSAME");
            hInvMassLS[ipt]->SetLineColor(4);
            hInvMassLS[ipt]->SetLineWidth(4);
            //drawLatex(0.15,0.9,"UnLike Sign",22,0.06,1);
            //drawLatex(0.15,0.84,"Like Sign",22,0.06,2);
            drawLatex(0.55,0.85,"STAR Au+Au @ 200 GeV",22,0.05,1);
            drawLatex(0.55,0.76,Form("%s, D^{0} pos |Y| < 1.0",nameCent[icent]),22,0.05,4);
            drawLatex(0.55,0.68,Form("%.1f < p_{T} < %.1f GeV/c",nptbin[ipt],nptbin[ipt+1]),22,0.05,4);
            ...
            TLegend* legend2 = new TLegend(0.15,.73,.5,.9);
            ...
            sprintf(name,"%s/cent%s_pt_%.1f_%.1f.png",dir,nameCent1[icent],nptbin[ipt],nptbin[ipt+1]);
            c1->SaveAs(name);
        }
