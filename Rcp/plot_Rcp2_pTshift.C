...
void plot_Rcp2_pTshift()
{
   ...
   const int ncent = 5;
   const char nameCent[ncent][250] = {"0-10%", "10-20%", "20-40%", "40-60%", "40-80%"};
   const char nameCentXL[ncent][250] = {"0_10", "10_20", "20_40", "40_60", "40_80"};
   float NbinMean[ncent] = {938.80170, 579.89409, 288.35051, 91.37100, 56.99229};

   //Read spectra
   TGraphErrors* gD0err_xl[ncent];
   TGraphErrors* gD0sys_xl[ncent];
   TF1* fLevy[ncent];
   TFile* fin1 = new TFile("../ptShift/D0_Spectra_Run14HFT_neg.root");
   ...
   sprintf(name, "%s/D0_Rcp2_pTshift.pdf", dir);
   c1->SaveAs(name);

   // ==== Write
   TFile* fout = new TFile("D0_Rcp2_pTshift.root", "RECREATE");
   fout->cd();
   for (int icent = 0; icent < ncent; icent++)
   {
