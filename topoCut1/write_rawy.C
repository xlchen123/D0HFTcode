...
void write_rawy()
{
    ...
    THnF* hnmassMixUL = (THnF*)fin->Get("hD0CentPtEtaMDphiDaugMixed_50efficiency");
    //hnmassMixUL->SetDirectory(0);
    THnF* hnmassMixLS = (THnF*)fin->Get("hD0CentPtEtaMDphiDaugLikeSignMixed_50efficiency");
    ...
    for(int icent=0; icent<ncent; icent++) {
        ...
        for(int ipt=0; ipt<npt; ipt++) {
            ...
            //cout << hInvMassLS->Integral(mbin_lw,mbin_up)  << "\t" << hInvMassMixLS->Integral(mbin_lw,mbin_up) << "\t" << hInvMassMixUL->Integral(mbin_lw,mbin_up) << "\t" <<  norm << endl;
