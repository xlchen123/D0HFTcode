...
TH1F* hmLike;
TF1* fmLike;
double hmLiketoF(double x) {
    return hmLike->GetBinContent(hmLike->FindBin(x));
}
...
void plot_rawy()
{
    ...
    for(int icent=0; icent<ncent; icent++) {
        ...
        double S, B;
        for(int ipt=0; ipt<npt; ipt++) {
            if(hmLike) delete hmLike;
            hmLike = (TH1F*)hInvMassMixUL[ipt]->Clone();
            ...
            legend2->AddEntry(hInvMassMixUL[ipt]," US (Mix-Event)","lep");
            ...
            hInvMass[ipt]->SetMaximum(1.8*hInvMass[ipt]->GetMaximum());
            ...
            hInvMass[ipt]->Fit(fD0,"LNOR","",fitRange_lw, fitRange_up);
            hInvMass[ipt]->Fit(fD0,"LNOR","",fitRange_lw, fitRange_up);
            ...
            lowx = mean - 3.0*sigma;
            higx = mean + 3.0*sigma;
            gausfun->SetParameters(par);
            resifun->SetParameters(&par[4]);
            gausfun->SetLineColor(2);
            resifun->SetLineColor(7);
            /*TH1F* hD0_clone = (TH1F*)hInvMass[ipt]->Clone(Form("hD0Clone%i%i",icent,ipt));
            hD0_clone->Add(resifun,-1);
            gausfun->FixParameter(3,hInvMass[ipt]->GetBinWidth(4));
            hD0_clone->Fit(gausfun,"INOR","",fitRange_lw, fitRange_up);
            hD0_clone->Fit(gausfun,"INOR","",fitRange_lw, fitRange_up);
            hD0_clone->SetMarkerColor(2);
            hD0_clone->Draw("esame");
            gausfun->Draw("same");*/
            ...
            rawY[ipt] = fD0->GetParameter(0); rawYerr[ipt] = fD0->GetParError(0);
            ...
            fD0->DrawCopy("same");
            S = gausfun->Integral(lowx,higx)/hInvMass[ipt]->GetBinWidth(2);
            fmLike = new TF1("fmLike","hmLiketoF(x)",1,3);
            B = fmLike->Integral(lowx,higx)/hInvMass[ipt]->GetBinWidth(2) + resifun->Integral(lowx,higx)/hInvMass[ipt]->GetBinWidth(2);
            cout << "bin width: " << hInvMass[ipt]->GetBinWidth(2) << endl;
            cout << "S = " << S << endl;
            cout << "B = " << B << endl;
            int N_y = round(gausfun->GetParameter(0));
            int N_error = round(gausfun->GetParError(0));
            int Nwr = N_y - N_y%10;
