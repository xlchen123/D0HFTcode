...
void plot_Nsig_pion() {
    ...
    //pbin[2] = 0.3;
    //pbin[3] = 0.35;
    for(int i=7; i<24; i++) pbin[i] = 0.4+0.1*(i-7);
    pbin[24] = 2.2;
    pbin[25] = 2.5;
    pbin[26] = 3.0;
    pbin[27] = 4.0;*/
    ...
    pbin[2] = 0.3;
    pbin[3] = 0.35;
    for(int i=4; i<21; i++) pbin[i] = 0.4+0.1*(i-4);
    pbin[21] = 2.2;
    pbin[22] = 2.5;
    pbin[23] = 3.0;
    pbin[24] = 4.0;
    ...
    TFile* fin = new TFile("data/pion_noHFT.root");
    fin->cd();
    ...
    pad->Draw();
    TPDF *pdf = new TPDF("PDF/PionPid.pdf");
    pad->cd();
    ...
    sprintf(name, "Pion PID and efficiency");
    ...
    for(int i=0; i<np; i++) {
        ...
        setHisto(h1[ii], "", "n#sigma_{#pi}", "Counts");
        ...
        if(i==np-1) {h1[ii]->Fit(f1[ii],fitOp,"",-4,4);h1[ii]->Fit(f1[ii],fitOp,"",-4,4);h1[ii]->Fit(f1[ii],fitOp,"",-4,4);}
        else {h1[ii]->Fit(f1[ii],fitOp,"",-4,4);h1[ii]->Fit(f1[ii],fitOp,"",-4,4);h1[ii]->Fit(f1[ii],fitOp,"",-4,4);}
        ...
        h2[ii]->Rebin(nRebin);
        //h1[ii]->SetMinimum(minimum);
        setHisto(h2[ii], "", "n#sigma_{#pi}^{TOF}", "Counts");
        ...
        if(i==20) {h2[ii]->Fit(f2[ii],fitOp,"",-4,4);h2[ii]->Fit(f2[ii],fitOp,"",-4,4);h2[ii]->Fit(f2[ii],fitOp,"",-4,4);}
    ...
    h0->Draw();
    gr2->Draw("psame");
    sprintf(name,"n#sigma_{#pi} sigma");
    drawLatex(0.55,0.75,name,22,0.05,4);
    gPad->Update();
    ...
    h0->Draw();
    gr3->Draw("psame");
    sprintf(name,"n#sigma_{#pi}^{TOF} mean");
    drawLatex(0.55,0.75,name,22,0.05,4);
    gPad->Update();
    ...
    h0->Draw();
    gr4->Draw("psame");
    sprintf(name,"n#sigma_{#pi}^{TOF} sigma");
    drawLatex(0.55,0.75,name,22,0.05,4);
    gPad->Update();
    ...
    h0->SetMinimum(0.7),
    h0->SetMaximum(1.02);
    setHisto(h0,"","p (GeV/c)", "n#sigma_{#pi}^{TOF} cut eff");
    //setHisto(h0,"","p (GeV/c)", "ratio",1,0.05,0.05, 1,0.9,1.1, 1,0,3.2,0.,2);
    ...
    h0->Draw();
    gr6->Draw("psame");
    sprintf(name,"n#sigma_{#pi}^{TOF} eff");
    drawLatex(0.55,0.75,name,22,0.05,4);
    ll->Draw("same");
    gPad->Update();
    ...
    ofstream out1("data/pionPid_eff.txt");
