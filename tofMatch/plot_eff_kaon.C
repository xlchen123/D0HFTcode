...
void plot_eff_kaon() {
    ...
    string str;
    float n_tmp;
    ifstream in("data/kaonPid_eff.txt");
    getline(in,str);
    cout << str << endl;
    in >> n_tmp;
    const int npt = n_tmp;
    ...
    TF1* fTofEff = new TF1("fTofEff","[0]+[1]/x-[2]/x/x+[3]/x/x/x",0,4);
    fTofEff->SetParameters(1,0.1,0.1,0.1);
    fTofEff->SetLineColor(kRed);
    fTofEff->SetLineWidth(linewidth);
    
    // fit
    const char fitOpt[10] = "NOR";
    const float fitR_lw = 0.25;
    const float fitR_up = 3.;
    const float fitR_lw1 = 0.2;
    const float fitR_up1 = 1.;
    ...
    // plot
    TH1F* h0 = new TH1F("","",1,0,3);
    setHisto(h0, "", "p (GeV/c)", "n#sigma_{k} cut eff.");
    h0->GetYaxis()->SetRangeUser(0.7,1.02);
    h0->Draw();
    gTpcEff->Draw("psame");
    fTpcEff->Draw("same");
    sprintf(name,"Kaon TPC PID eff. = %.3f",fTpcEff->GetParameter(0));
    drawLatex(0.2,0.2,name,132,0.05,1);
    c1->SaveAs("pic/TpcPidEff_k.gif");
    
    TH1F* h1 = new TH1F("","",1,0,3);
    setHisto(h1, "", "p (GeV/c)", "1/#beta_{k} cut eff.");
    h1->GetYaxis()->SetRangeUser(0.7,1.02);
    h1->Draw();
    gTofEff->Draw("psame");
    //fTofEff->Draw("same");
    sprintf(name,"Kaon TOF PID eff.");
    drawLatex(0.2,0.2,name,132,0.05,1);
    c1->SaveAs("pic/TofPidEff_k.gif");
    
    // write to root file
    TFile* fout = new TFile("data/PIDeff_k.root","RECREATE");
    gTpcEff->Write("gTpcEff_k");
    gTofEff->Write("gTofEff_k");
    fout->Close();
}
